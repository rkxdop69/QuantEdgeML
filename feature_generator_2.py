...
def feature_gen(data:pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        df["adfuller"]=df["close"].rolling(window=14).apply(lambda x: adfuller(x)[0], raw=True)
        df["acf"]=df["close"].rolling(window=14).apply(lambda x: acf(x, nlags=1)[1], raw=True)
        df["pacf"]=df["close"].rolling(window=14).apply(lambda x: pacf(x, nlags=1)[1], raw=True)
        slopes=[]
        ...
        df["f1"]=(df["high"]-df["close"])/(df["open"]-df["low"])
        df["f2"]=(df["high"]-df["open"])/(df["close"]-df["low"])
        df["f3"]=(df["high"]-df["low"])/(df["open"]-df["close"])
        df["f4"]=df["ema10"]/df["ema5"]
        df["f5"]=df["RSI"]/100
        df["f6"]=df['macd']/df["signal_macd"]
        df["f7"]=df["%k"]-df["perd"]
        df["f8"]=df["lin_reg"]/df["close"].mean()
        df["f9"]=(df["ubb"]-df["lbb"])/(df["high"]-df["low"])
        df["f10"]=(df["ubb"]-df["close"])/(df["close"]-df["lbb"])
        df["f11"]=df["%k"]
        df["f12"]=df["acc"]/df["close"].mean()
        df["f13"]=df["acf"]
        df["f14"]=df["adfuller"]
        df["f15"]=df["pacf"]

        return df.loc[:, "f1":"f15"]
